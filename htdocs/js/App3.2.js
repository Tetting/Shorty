/**
Update to support jQuery 1.6+
All .attr should be moved to .prop
*/
/**
...
var App3 = {
	...
	,loadScript:function(src,OnLoadedHandler,forceRead,OnErrorHandler) {
		...
		function LoadError() {
			if (console) console.error("Failed to load:"+this.src);
			...
			.find('script').each(function(){
				if ( this.src ){ jQuery.getScript( this.src ); }else{
				 	try{jQuery.globalEval( this.text || this.textContent || this.innerHTML || '' )}catch(e){
				    	if(console) console.error("Script Eval Failed:",basehref,paramurl,e,this.text);	
				   	}
				} 
			});
				...
				*/
				...
				$j('.ajaxComp',document.getElementById(targetDivId))
					.each(function() {
						...
						var nav = $j(this).prop("uri");
						/* expensive but pretty/ easy */
						var params = {};
						if ($j(this).prop("params")) {
							try {
								eval("params ="+$j(this).prop("params"));	
							} catch (e){
								console.error("ajaxComp params error:",$j(this).prop("params"),e);	
							}
						}
						//default target of component is the tag with the ajaxComp class on it.
						if (!params.target){params.target = $j(this).prop("id");}
						//target attribute over-rides JSON params attribute :-)
						if ($j(this).prop("target")) {params.target = $j(this).prop(target);} 
			...
			//console.info("404:",links,loc,links[pos],pos,params,_404loopcount);
				...
				do {
					...
					if (pos > -1) {
						...
						loc = l;
						if (console) console.log("404:",links[pos],loc);
					} 
					/** -- this can be removed..
					else {
						//we are looking at the host call.
	...
	*/ 
	,globalAjaxEventHandler:function(ein) {
		...
		//console.info('click',$j(el).prop('className'),el);
		
		var matchFn = function() {
			if ($j(this).prop("className").indexOf('ajaxEdit3')>-1)	{
				/* we should edit this data. */
				//console.info("ajaxEdit3");
				EditInPlace4.dataViewEdit(this);
			} else {
				if ($j(this).prop("className").indexOf('ajaxEdit')>-1)	{
					/* we should edit this data. */
			...
			if ($j(this).prop("className").indexOf('ajaxLink3')>-1)	{
		...
		/**
		if (\$j(el).prop('class')) {
			if (\$j(el).prop('className').indexOf('ajaxEdit')>-1) {
	...
	*/
	,Remote404:function(ns) {
		...
		params = ns;
		if(console) console.info("404 Error:",params);
