...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  // 'NewPageCallback'=>functionName, or array(object,functionName) called when table goes onto new page,
  //		done so you can print a limited number of records.
  //
...
//        $this->reopenObject($textObjectId);
    ...
    if ($options['splitRows']==0){
      if ( ( ($this->ezPageCount != $pageStart) || (isset($this->ez['columns']) && $this->ez['columns']['on']==1 && $columnStart != $this->ez['columns']['colNum'] ))  && $secondTurn==0){
        ...
        $x0=$baseX0+$dm;
        $x1=$baseX1+$dm;
		if (isset($options['NewPageCallback'])) {
			$command = call_user_func($options['NewPageCallback'], $cnt);
			switch($command) {
				case 'Bailout': 
					//copy of code after foreach
					$this->transaction('commit');
					$y2=$y+$decender;
					if ($options['showLines']){
						if (!$options['showHeadings']){
