...
while($row) {    $font = Zend_Pdf_Font::fontWithPath($reportPath.'/fonts/arialbd.ttf',Zend_Pdf_Font::EMBED_SUPPRESS_EMBED_EXCEPTION); 
    ...
    alignedText(array('y'=>'250','xStart'=>'56','xEnd'=>'530','align'=>'left'),utf8_decode("Faktura"),$page,$offsety);    $page->setFont($page->getFont(), 12);
    ...
    $page->setFont($font, 12);
    $page->setFont($page->getFont(), 10);
    alignedText(array('y'=>'240','xStart'=>'56','xEnd'=>'200','align'=>'left'),utf8_decode("Nr 00000150"),$page,$offsety);    $row = $db->GetRow($result);
    if ($row) {
        $page = new Zend_Pdf_Page($templatePage);
        $pdf->pages[] = $page;
        $page->setFont(Zend_Pdf_Font::fontWithName(Zend_Pdf_Font::FONT_HELVETICA), 12);
