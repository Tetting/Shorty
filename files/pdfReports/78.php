...
<tr><th colspan="2">Påminnelse</th></tr>
<tr><td colspan="2">Betalningspåminnelse</td></tr><tr><td>Givare#:</td><td><input name="id" value="<?
...
if(isset($_REQUEST['letter'])) {
    ...
    print 'Enligt våra noteringar underhåller ni 

Totalt saknas betalning för X månader á X kr/mån';
}
?></textarea></td></tr>
...
while($row) {    $font = Zend_Pdf_Font::fontWithPath($reportPath.'/fonts/arial.ttf',Zend_Pdf_Font::EMBED_SUPPRESS_EMBED_EXCEPTION); 
    ...
    $t = array('y'=>575,'align'=>'left','xStart'=>92,'xEnd'=>'530');
    alignedText($t,dbTexts('Name',$row,' '),$page,$offsety);
    $page->setFont($page->getFont(), 12);
    alignedText(array('y'=>'438','xStart'=>'57','xEnd'=>'490','align'=>'left'),utf8_decode("$letter = $sum kr"),$page,$offsety);    $t = array('y'=>229,'align'=>'left','xStart'=>141,'xEnd'=>'530');
    alignedText($t,dbTexts('Id',$row,' '),$page,$offsety);
    $page->setFont($page->getFont(), 12);
    alignedText(array('y'=>'201','xStart'=>'156','xEnd'=>'530','align'=>'left'),utf8_decode("$sum kr"),$page,$offsety);    $page->setFont($page->getFont(), 12);
    alignedText(array('y'=>'180','xStart'=>'57','xEnd'=>'530','align'=>'left'),utf8_decode("Projekt: $projectId"),$page,$offsety);    $font = Zend_Pdf_Font::fontWithPath($reportPath.'/fonts/arialbd.ttf',Zend_Pdf_Font::EMBED_SUPPRESS_EMBED_EXCEPTION); 
    $page->setFont($font, 12);
    $page->setFont($page->getFont(), 12);
...
//output pdf inline
$dfile = "Påminnelse$id.pdf";
if (strpos($id,',')>0) {
    $dfile = "Påminnelse.pdf";
}
