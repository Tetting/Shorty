...
<tr><th colspan="2">Logi Ralle</th></tr>
<tr><td colspan="2">Faktura för logi på Smålandskonferensen.</td></tr><tr><td>Givare#:</td><td><input name="id" value="<?
...
if(isset($_REQUEST['letter'])) {
    ...
    print 'Vi har bokat ett rum i X och noterat att ni är med hela veckan.';
}
?></textarea></td></tr>
...
while($row) {    $font = Zend_Pdf_Font::fontWithPath($reportPath.'/fonts/arial.ttf',Zend_Pdf_Font::EMBED_SUPPRESS_EMBED_EXCEPTION); 
    ...
    $t = array('y'=>648,'align'=>'left','xStart'=>92,'xEnd'=>'530');
    alignedText($t,dbTexts('Name',$row,' '),$page,$offsety);
    $page->setFont($page->getFont(), 12);
    alignedText(array('y'=>'606','xStart'=>'57','xEnd'=>'480','align'=>'wrap'),utf8_decode("$letter"),$page,$offsety);    $t = array('y'=>234,'align'=>'left','xStart'=>141,'xEnd'=>'530');
    alignedText($t,dbTexts('Id',$row,' '),$page,$offsety);
    $page->setFont($page->getFont(), 12);
    alignedText(array('y'=>'193','xStart'=>'92','xEnd'=>'530','align'=>'left'),utf8_decode("$sum kr"),$page,$offsety);    $font = Zend_Pdf_Font::fontWithPath($reportPath.'/fonts/arialbd.ttf',Zend_Pdf_Font::EMBED_SUPPRESS_EMBED_EXCEPTION); 
    $page->setFont($font, 12);
    $page->setFont($page->getFont(), 12);
    ...
    alignedText(array('y'=>'680','xStart'=>'400','xEnd'=>'530','align'=>'right'),utf8_decode("Bollnäs ".date("Y-m-d").""),$page,$offsety);    $row = $db->GetRow($result);
    if ($row) {
        $page = new Zend_Pdf_Page($templatePage);
        $pdf->pages[] = $page;
        $page->setFont(Zend_Pdf_Font::fontWithName(Zend_Pdf_Font::FONT_HELVETICA), 12);
...
//output pdf inline
$dfile = "Logi Ralle$id.pdf";
if (strpos($id,',')>0) {
    $dfile = "Logi Ralle.pdf";
}
