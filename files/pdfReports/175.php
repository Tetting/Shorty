...
while($row) {
    ...
    $t = array('y'=>583,'align'=>'left','xStart'=>90,'xEnd'=>'200');
    alignedText($t,dbTexts('Name',$row,' '),$page,$offsety);
    $page->setFont($page->getFont(), 12);
    alignedText(array('y'=>'564','xStart'=>'54','xEnd'=>'485','align'=>'wrap'),utf8_decode("$letter"),$page,$offsety);    $page->setFont($page->getFont(), 10);
    alignedText(array('y'=>'625','xStart'=>'400','xEnd'=>'530','align'=>'right'),utf8_decode("Bollnäs ".date("Y-m-d").""),$page,$offsety);    $font = Zend_Pdf_Font::fontWithPath($reportPath.'/fonts/arialbd.ttf',Zend_Pdf_Font::EMBED_SUPPRESS_EMBED_EXCEPTION); 
    $page->setFont($font, 12);
...
//output pdf 
$dfile = "Fritt brev (inbet)$id.pdf";
if (strpos($id,',')>0) {
    $dfile = "Fritt brev (inbet).pdf";
}
