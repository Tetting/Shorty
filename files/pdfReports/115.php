...
<tr><th colspan="2">Mat  för Dagen</th></tr>
<tr><td colspan="2">Detta brev skickas till givare som ger engångsgåva till Mat för Dagen.</td></tr><tr><td>Givare#:</td><td><input name="id" value="<?
...
while($row) {    $font = Zend_Pdf_Font::fontWithPath($reportPath.'/fonts/arial.ttf',Zend_Pdf_Font::EMBED_SUPPRESS_EMBED_EXCEPTION); 
    ...
    alignedText(array('y'=>'208','xStart'=>'135','xEnd'=>'530','align'=>'left'),utf8_decode("$sum kr"),$page,$offsety);    $font = Zend_Pdf_Font::fontWithPath($reportPath.'/fonts/arialbd.ttf',Zend_Pdf_Font::EMBED_SUPPRESS_EMBED_EXCEPTION); 
    $page->setFont($font, 12);
    $page->setFont($page->getFont(), 12);
...
//output pdf inline
$dfile = "Mat för Dagen$id.pdf";
if (strpos($id,',')>0) {
    $dfile = "Mat för Dagen.pdf";
}
