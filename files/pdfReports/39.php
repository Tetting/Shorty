...
<tr><th colspan="2">Fadderavslut - ej betalt</th></tr>
...
?>"/></td></tr>
<tr><td>Betalning fattas för:</td><td><input name="months" value="<?
if(isset($_REQUEST['months'])) {
    print $_REQUEST['months'];
} else {
    print '';
}
?>"/></td></tr>
...
$months="";
if (isset($_REQUEST['months'])) {
    $months = utf8_encode(str_replace("
".PHP_EOL,PHP_EOL,$_REQUEST['months']));
    if ($months == "Test123") {
        $months="Skriv egen text här";   
    }
}    $query="Select 
     Giver.* 
...
while($row) {    $font = Zend_Pdf_Font::fontWithPath($reportPath.'/fonts/arial.ttf',Zend_Pdf_Font::EMBED_SUPPRESS_EMBED_EXCEPTION); 
    ...
    alignedText(array('y'=>'540','xStart'=>'207','xEnd'=>'480','align'=>'left'),utf8_decode("$months"),$page,$offsety);    $page->setFont($page->getFont(), 10);
...
//output pdf inline
$dfile = "Fadderavslut - ej betalt$id.pdf";
if (strpos($id,',')>0) {
    $dfile = "Fadderavslut - ej betalt.pdf";
}
