...
<tr><th colspan="2">Utbetalningar per projekt</th></tr>
<tr><td colspan="2">Skapar lista med utbetalningar för angivet projekt.</td></tr><tr><td>Projekt#:</td><td><input name="id" value="<?
...
Where
    ...
    $data = array();$tot = 0;
    while($row = $db->GetRow($result)) {$tot += $row["Payment.OutKr"];
        $data[] = $row;
    }
    
    //add a totals row...
    $row = array();
    $row["Payment.OutKr"] = "<b>".$tot."</b>";$row["Giver.Id"] = "Summa";
    $data[] = $row;
    ...
    $pdf->ezTable(
	    ...
	    ,html_entity_decode("Utbetalningar f&ouml;r projekt $id")
        //options
