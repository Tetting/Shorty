...
<tr><th colspan="2">Barnbyte (ID)</th></tr>
<tr><td colspan="2">Detta brev skickas vid barnbyte.<br /><br />
Fyll i barn ID och ändra det som behövs i texten.</td></tr><tr><td>Barn ID:</td><td><input name="id" value="<?
...
?>"/></td></tr>
<tr><td>Orsak till avslut:</td><td><textarea rows="15" cols="110" name="text"><?
if(isset($_REQUEST['text'])) {
    print $_REQUEST['text'];
} else {
    print 'Som tack för ert trogna stöd ska barnen skriva rapporter till sina faddrar. Eftersom inga rapporter inkommit från barnet på två år, trots flera påminnelse, har vi beslutat att byta till ett annat barn.

Med i detta brev finns kort och information på ett nytt barn som vi önskar att ni blir fadder för. Om ni inte önskar ett nytt barn, var vänlig och kontakta oss snarast, annars övergår underhållet automatiskt till det nya barnet! (Vi behöver få tillbaka info + foto om ni inte vill fortsätta fadderskapet).

Med önskan om fortsatt gott samarbete!


     Jeanett Åsblom                          Veronika Bodén                       Siv Sandsgård
jeanett@trosgnistan.se             veronika@trosgnistan.se             siv@trosgnistan.se
';
}
?></textarea></td></tr>
...
<?
  ...
  $id = $_REQUEST['id'];
 $text = utf8_encode($_REQUEST['text']);
}

$text="";
if (isset($_REQUEST['text'])) {
    $text = utf8_encode(str_replace("".PHP_EOL,PHP_EOL,$_REQUEST['text']));
    if ($text == "Test123") {
        $text="Skriv egen text här...";   
    }
}
...
while($row) {    $font = Zend_Pdf_Font::fontWithPath($reportPath.'/fonts/arial.ttf',Zend_Pdf_Font::EMBED_SUPPRESS_EMBED_EXCEPTION); 
    ...
    $t = array('y'=>579,'align'=>'left','xStart'=>92,'xEnd'=>'530');
    alignedText($t,dbTexts('Giver.Name',$row,' '),$page,$offsety);
    $t = array('y'=>552,'align'=>'left','xStart'=>290,'xEnd'=>'530');
    alignedText($t,dbTexts('Fadderbarn.Name',$row,' '),$page,$offsety);
    $page->setFont($page->getFont(), 12);
    alignedText(array('y'=>'528','xStart'=>'58','xEnd'=>'480','align'=>'wrap'),utf8_decode("$text"),$page,$offsety);    $page->setFont($page->getFont(), 10);
...
//output pdf inline
$dfile = "Barnbyte (ID)$id.pdf";
if (strpos($id,',')>0) {
    $dfile = "Barnbyte (ID).pdf";
}
