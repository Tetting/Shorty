...
$result = $db->Query($query);
    while($row = $db->GetRow($result)) {
        ...
        AND action.actionName in ('Ny rapport')
        order by date desc
        limit 0,1
        )
...
From 
    ...
    , Giver 
Where 
    (Fadderbarn.Status is null OR Fadderbarn.Status != 'Inaktiv')
    
	Order by Fadderbarn.Nbr ASC
";
    ...
    $pdf->ezText("");
    $pdf->ezSetDy(0);
    $pdf->ezText("<b>Children list $area $areaName</b>");
    $pdf->ezTable(
        $data
        //columns
