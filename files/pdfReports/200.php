...
<tr><td colspan="2">Skriver ut lista på givare som betalat till angivet projekt de sista 12 månaderna.</td></tr><tr><td>Projekt#:</td><td><input name="id" value="<?
...
<?
  ...
  $id = $_REQUEST['id'];
    ...
    $id = $_REQUEST['id'];    $query="SELECT
      ...
      ,Giver.*
      ,ifnull(Giver.Name,'')||ifnull(' '||Giver.LastName,'') || '  ' || Giver.coAddress || '  ' || Giver.Address || ',  ' || Giver.ZipCode || '  ' || Giver.ZipTown as 'Giver.FullName'
FROM
...
Order by Payment.GiverId";
    ...
    $pdf->ezTable(
	    ...
	    ,html_entity_decode("<b>Inbetalt till projekt $id</b>")
        //options
