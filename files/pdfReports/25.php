...
<tr><th colspan="2">Barnrapport - allmän</th></tr>
<tr><td colspan="2">Skriver ut brev som skickas med allmän rapport från faddebarn.<br /><br/><b><i>OBS när du klickar på 'Kör' läggs datum för rapport till barnet.</B></I></td></tr><tr><td>Barn#:</td><td><input name="id" value="<?
...
$db->Insert(array(
    'table'=>'action',
    'fields'=>array(
        'actionName'=>'Utskrift Barnrapport - allmän'
        ,'entityName'=>'Fadderbarn'
        ,'entityId'=>$row['Fadderbarn.Id']
        ,'date'=>date('Y-m-d', time())
        ,'notes'=>'Utskrift Barnrapport - allmän'
    )
    ...
    $t = array('y'=>500,'align'=>'left','xStart'=>222,'xEnd'=>'530');
    alignedText($t,dbTexts('Fadderbarn.Area',$row,' '),$page,$offsety);
    $row = $db->GetRow($result);
    if ($row) {
        $page = new Zend_Pdf_Page($templatePage);
        $pdf->pages[] = $page;
...
//output pdf inline
$dfile = "Barnrapport - allmän$id.pdf";
if (strpos($id,',')>0) {
    $dfile = "Barnrapport - allmän.pdf";
}
